...
module.exports = {
  ...
  async execute(interaction, _args, client) {
    ...
    if (!Number.isFinite(value) || value < 0 || value > 600) {
      return interaction.reply({ content: '❌ Введите число от 0 до 600.', flags: MessageFlags.Ephemeral });
    }
    ...
    try {
      ...
      await interaction.reply({ content: '✅ Кулдаун голоса обновлен.', flags: MessageFlags.Ephemeral });
    } catch (e) {
      client.logs?.error?.(`Leveling set voice cooldown modal error: ${e.message}`);
      await interaction.reply({ content: '❌ Ошибка при сохранении настройки.', flags: MessageFlags.Ephemeral });
    }
  }
};
