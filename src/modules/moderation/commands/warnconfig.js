...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('warnconfig')
    .setDescription('Настройка наказаний за предупреждения')
    .addSubcommand(sub => sub
      .setName('view')
      .setDescription('Показать текущие настройки'))
    .addSubcommand(sub => sub
      .setName('set')
      .setDescription('Установить порог наказания')
      .addStringOption(o => o.setName('action').setDescription('Тип наказания').setRequired(true).addChoices(
        { name: 'Мут', value: 'mute' },
        { name: 'Кик', value: 'kick' },
        { name: 'Бан', value: 'ban' }
      ))
      .addIntegerOption(o => o.setName('count').setDescription('Количество предупреждений').setRequired(true))
      .addIntegerOption(o => o.setName('duration').setDescription('Длительность мута в минутах').setRequired(false))),
  guildOnly: true,
  cooldown: 3,
  ...
  async execute(interaction, client) {
    ...
    if (sub === 'view') {
      ...
      return interaction.reply({
        content: [
          `Мут: ${cfg?.muteThreshold ?? 3} предупреждений (на ${cfg?.muteDurationMin ?? 60} мин.)`,
          `Кик: ${cfg?.kickThreshold ?? 5} предупреждений`,
          `Бан: ${cfg?.banThreshold ?? 7} предупреждений`
        ].join('\n'),
    ...
    if (sub === 'set') {
      ...
      if (count <= 0) {
        return interaction.reply({ content: 'Количество должно быть больше 0.', flags: MessageFlags.Ephemeral });
      }
