...
module.exports = {
  ...
  async execute(interaction, args, client) {
    ...
    try {
      ...
      const cfg = (await svc.getSettings(guildId)) || {};
      ...
      const nextCfg = { ...cfg, [field]: next };
      await svc.setSettings(guildId, nextCfg);
    } catch (e) {
    ...
    try {
      ...
      let cfg2 = null;
      try {
        const WarnSettingsService = require('../../services/WarnSettingsService');
        const svc = WarnSettingsService();
        cfg2 = await svc.get(guildId);
      } catch {}
