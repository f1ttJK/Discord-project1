...
async function buildGeneralContainers(interaction, client) {
  ...
  try {
    ...
    cfg = await svc.getSettings(guildId);
  ...
  async execute(interaction, args, client) {
    ...
    try {
      if (target === 'warn') {
        const WarnService = require('../../services/WarnService');
        const svc = WarnService();
        const cur = await svc.getSettings(guildId);
        const current = cur?.enabled !== false; // default true
        await svc.setSettings(guildId, { ...cur, enabled: !current });
      } else if (target === 'mute') {
        const guildRow = await client.prisma.guild.findUnique({ where: { id: guildId } }).catch(() => null);
