  ...
  async execute(interaction, client) {
    ...
    try {
      if (reason.punishmentType === 'Mute') {
        ...
        if (!roleId) {
          return interaction.reply({ content: '-  .    /warnconfig setmuterole.', flags: MessageFlags.Ephemeral });
        }
        const role = guild.roles.cache.get(roleId) || await guild.roles.fetch(roleId).catch(() => null);
        if (!role) return interaction.reply({ content: ' -  .', flags: MessageFlags.Ephemeral });
        const me = guild.members.me;
        if (!me) return interaction.reply({ content: '     .', flags: MessageFlags.Ephemeral });
        if (role.position >= me.roles.highest.position) {
          return interaction.reply({ content: '    :      .', flags: MessageFlags.Ephemeral });
        }
        await member.roles.add(role, `  : ${reason.label}`);
        applied = true;
        actionText = `  ( <@&${role.id}>)  ${durationMin} .`;
        setTimeout(async () => {
          const fresh = await guild.members.fetch(targetUser.id).catch(() => null);
          if (!fresh) return;
          if (fresh.roles.cache.has(role.id)) {
            fresh.roles.remove(role, ' ').catch(() => null);
          }
        }, durationMin * 60 * 1000);
      } else {
        const ms = durationMin * 60 * 1000;
        await member.timeout(ms, `  : ${reason.label}`);
        applied = true;
        actionText = `Timeout  ${durationMin} .`;
      }
    } catch (e) {
      client.logs?.warn && client.logs.warn(`   : ${e.message}`);
    }
    ...
    if (applied && logChannelId) {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: '', value: reason.label, inline: false },
          { name: '', value: `${durationMin} .`, inline: true },
          { name: '', value: reason.punishmentType === 'Mute' ? 'Role Mute' : 'Timeout', inline: true },
        )
        .setTimestamp();
    ...
    return interaction.reply({ content: applied ? `  : ${actionText}` : '    .', flags: MessageFlags.Ephemeral });
  }
};
