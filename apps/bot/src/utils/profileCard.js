...
function formatJoinedDurationUA(joinMs, nowMs = Date.now()) {
  ...
  const year = Math.floor(diff / (365 * 24 * 3600)); if (year >= 1) return `${year} ${pluralUA(year, 'i', '', 'i')}`
  const month = Math.floor(diff / (30 * 24 * 3600)); if (month >= 1) return `${month} ${pluralUA(month, 'i', 'ii', 'ii')}`
  const week = Math.floor(diff / (7 * 24 * 3600)); if (week >= 1) return `${week} ${pluralUA(week, 'i', 'ii', 'i')}`
  const day = Math.floor(diff / (24 * 3600)); if (day >= 1) return `${day} ${pluralUA(day, '', 'i', 'i')}`
  const hour = Math.floor(diff / 3600); if (hour >= 1) return `${hour} ${pluralUA(hour, '', '', '')}`
  const minute = Math.floor(diff / 60); if (minute >= 1) return `${minute} ${pluralUA(minute, '', '', '')}`
  const second = diff; return `${second} ${pluralUA(second, '', '', '')}`
}
...
function sampleCanvasColor(ctx, x, y) {
  ...
  const a = (typeof d[3] === 'number' ? d[3] : 255) / 255
...
async function renderCard(opts = {}) {
  ...
  if (opts.roleName) {
    ...
    // Font stack with symbol fallback so UTF-8 ( )    
    const family = GlobalFonts.has('Montserrat SemiBold')
  ...
  //  balance box: x=665 y=68 w=196 h=36, color #FFD642
  {
  ...
  // i balance box: x=665 y=128 w=196 h=36, color #FFFFFF
  {
