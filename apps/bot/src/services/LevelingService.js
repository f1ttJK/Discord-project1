...
async function addMessageXp(client, guildId, userId, now = new Date(), opts = {}) {
  const baseXp = (opts.baseMsgXp === null || opts.baseMsgXp === undefined) ? DEFAULTS.baseMsgXp : opts.baseMsgXp;
  const cooldownMs = (opts.msgXpCooldownMs === null || opts.msgXpCooldownMs === undefined) ? DEFAULTS.msgXpCooldownMs : opts.msgXpCooldownMs;
  const key = `${guildId}:${userId}`;
  const lastVal = lastMsgXpAt.get(key);
  const last = (lastVal === null || lastVal === undefined) ? 0 : lastVal;
...
async function addVoiceXp(client, guildId, userId, seconds, opts = {}) {
  const perMin = (opts.baseVoiceXpPerMinute === null || opts.baseVoiceXpPerMinute === undefined) ? DEFAULTS.baseVoiceXpPerMinute : opts.baseVoiceXpPerMinute;
